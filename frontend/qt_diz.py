...
from PyQt5.QtWidgets import (
    ...
    QLabel, QHBoxLayout, QPushButton, QProgressBar, QSizePolicy, QLineEdit, QDialog, QMessageBox, QInputDialog, QFileDialog
)
...
class MinimalAddButton(QPushButton):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(44, 44)
        self.setCursor(Qt.PointingHandCursor)
        self.setToolTip("Добавить файлы")
        self.setStyleSheet("""
            QPushButton {
                background: #101012;
                border: none;
...
class UploadProgressBar(QProgressBar):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimum(0)
        self.setMaximum(100)
...
class FileListWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.add_button = MinimalAddButton()
        upload_row.addWidget(self.add_button)

        self.upload_progress = UploadProgressBar()
        self.upload_progress.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.upload_progress.setFixedHeight(44)
                ...
                try:
                    with open(file_path, "rb") as f:
                        ...
                        if resp.status_code != 200:
                            ...
                            CustomMessageBox.warning(self, "Ошибка", f"Ошибка загрузки: {resp.text}")
                            return
                except Exception as e:
                    self.upload_progress.setValue(0)
                    self.upload_button.setEnabled(True)
                    CustomMessageBox.warning(self, "Ошибка", f"Ошибка: {e}")
                    return
